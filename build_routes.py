...
def builder():
    components = Component.query.order_by(Component.category).all()
...
def saved_builds():
    ...
    return render_template("builder/saved_builds.html", builds=builds)
...
def export_build(build_id):
    ...
    for cid in component_ids:
        c = comp_map.get(cid)
        if c:
            lines.append(f"{c.category:<14} {c.name} ({c.brand})  —  ₱{c.price:,.0f}")
            total += c.price

    lines.append("=" * 40)
    lines.append(f"{'TOTAL':<14} ₱{total:,.0f}")
