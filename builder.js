...
const selectedParts = {};  // { category: { id, name, brand, price } }
...
function filterComponents(query) {
  document.querySelectorAll(".component-card").forEach(card => {
    ...
    const matchName = !query || card.dataset.name.toLowerCase().includes(query.toLowerCase());
...
function checkCompat() {
  ...
  fetch("/api/compatibility", {
    ...
    .then(data => {
      ...
      el.textContent = data.compatible
...
function exportBuild() {
  ...
  ORDER.forEach(cat => {
    const p = selectedParts[cat];
    if (p) {
      lines.push(`${cat.padEnd(14)} ${p.name} (${p.brand})  —  ₱${p.price.toLocaleString()}`);
      total += p.price;
    } else {
  ...
  lines.push(sep);
  lines.push(`${"TOTAL".padEnd(14)} ₱${total.toLocaleString()}`);
